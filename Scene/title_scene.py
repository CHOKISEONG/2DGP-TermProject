...
mouse_y = 0.0
def init():
    global image, image_white, image_white_frame, sound, crowd, img_timer, df, start_button, font
...
def draw():
    global cur_time, mouse_x, mouse_y
    clear_canvas()
    ...
    if img_timer >= 6.0:
        ...
        start_button.clip_draw(0,30,100,50,400,200, 220,100)
    else:
...
def update():
    global img_timer, image_white_frame, df, mouse_x, mouse_y
    cur_time = get_time()
    ...
    for event in events:
        if event.type == SDL_MOUSEMOTION:
            mouse_x, mouse_y = event.x, get_canvas_height() - event.y  # SDL 이벤트의 좌표 (원점: 윗쪽)
            if mouse_y < 200: mouse_y = 200
            if mouse_x < 100: mouse_x = 100
            if mouse_x > 500: mouse_x = 500
